...
class DocumentRAGSystem:
    def __init__(
        self, embedder=None, chat_client=None, cacheDir="cache", dataDir="data"
    ):
    ...
    def initialize(self):
        ...
        if (
            file_changes["is_valid"]
            and not file_changes["changed_files"]
            and not file_changes["new_files"]
            and not file_changes["removed_files"]
        ):
            print("Using cached embeddings")
            self.ragPipeline = RAGPipeline.from_cache(
                cacheFile=f"{self.cacheDir}/embeddings.json",
                embedder=self._embedder,
                chat_client=self._chat_client,
            )
        elif file_changes["is_valid"] and (
            file_changes["changed_files"]
            or file_changes["new_files"]
            or file_changes["removed_files"]
        ):
            print(
                f"Updating embeddings for {len(file_changes['changed_files']) + len(file_changes['new_files'])} changed file(s)"
            )
    ...
    def data_pipeline(self):
        ...
        try:
            ...
            return RAGPipeline(
                ...
                embedder=self._embedder,
                chat_client=self._chat_client,
                cache_file=f"{self.cacheDir}/embeddings.json",
            )
    ...
    def get_file_changes(self):
        ...
        try:
            if not os.path.exists(
                f"{self.cacheDir}/file_metadata.json"
            ) or not os.path.exists(f"{self.cacheDir}/embeddings.json"):
                return {
                    "is_valid": False,
                    "changed_files": [],
                    "new_files": [],
            ...
            if (
                os.path.getsize(f"{self.cacheDir}/file_metadata.json") == 0
                or os.path.getsize(f"{self.cacheDir}/embeddings.json") == 0
            ):
                return {
                    "is_valid": False,
                    "changed_files": [],
    ...
    def incremental_update(self, file_changes):
        ...
        try:
            ...
            files_to_update = set(
                file_changes["changed_files"] + file_changes["new_files"]
            )
            ...
            for i, chunk in enumerate(existing_chunks):
                ...
                if (
                    chunk_path not in files_to_update
                    and chunk_path not in files_to_remove
                ):
            ...
            for datafile in self.allFolders.rglob("*.pdf"):
                ...
                if datafile not in files_to_update:
                    fileMetadata[datafile] = existing_metadata.get(
                        datafile,
                        {
                            "fileSize": os.path.getsize(datafile),
                            "fileModifiedTime": os.path.getmtime(datafile),
